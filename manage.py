...
cm.add(Command(
    ...
    lambda c: 'gunicorn -b {0}:{1} server:app'.format(
        c['host'], c['port']),
    {
        'FLASK_APP': FLASK_APP,
        'FLASK_DEBUG': 'true'
...
cm.add(Command(
    ...
    lambda c: 'python -m flask run --host={0} --port={1} --debugger --reload'.format(
        c['host'], c['port']),
    {
        'FLASK_APP': FLASK_APP,
        'FLASK_DEBUG': 'true'
...
cm.add(Command(
    ...
    lambda c: 'python -m flask run --host={0} --port={1} --no-debugger --no-reload'.format(
        c['host'], c['port']),
    {
        'FLASK_APP': FLASK_APP,
        'FLASK_DEBUG': 'true'
